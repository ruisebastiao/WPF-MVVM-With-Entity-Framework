...
namespace VH.UI.UserControls.Controls
{
    ...
    public class TimePicker : Control
    {
        ...
        {
            return Utilities.IsFormatStringValid(value);
        }
        ...
        public static readonly DependencyProperty PopupHeightProperty =
            DependencyProperty.Register("PopupHeight", typeof (double), typeof (TimePicker), new PropertyMetadata(default(double)));
        ...
        #region IsValueReadOnly

        public static readonly DependencyProperty IsValueReadOnlyProperty =
            DependencyProperty.Register("IsValueReadOnly", typeof (bool), typeof (TimePicker), new PropertyMetadata(default(bool)));

        public bool IsValueReadOnly
        {
            get { return (bool) GetValue(IsValueReadOnlyProperty); }
        ...
        private void OnKeyDown(object sender, KeyEventArgs e)
        {
            if (!IsOpen)
            {
                if (Utilities.KeyboardUtilities.IsKeyModifyingPopupState(e))
                {
                    IsOpen = true;
        ...
        private void ValidateTime(TimeSpan time)
        {
            if (time.TotalHours >= 24d)
                throw new ArgumentException("Time value cannot be greater or equal to 24 hours");
        }
        ...
        private TimeItem GetNearestTimeItem(TimeSpan time)
        {
            if (_timeListBox != null)
            {
                ...
                //They are all lower than the searched time. 
